...
public class DispatcherServlet extends HttpServlet {
     ...
     * - /student/selectList.do ---> com.kh.student.controller.StudentSelectListController
    ...
    public DispatcherServlet() throws Exception{
    	...
    	for(String url : urlSet) {
    		...
    		System.out.println("[디스패처서블릿] className = " + className); // properties의 url
    		...
    		if(url.startsWith("/student")) {
    			...
    			args[0] = studentService; 		// 생성자 호출시 전달한 인자
    		}
    		else if(url.startsWith("/emp")) {
    			param[0] = EmpService.class; // 생성자인자로 전달될 타입
    			args[0] = empService; 		// 생성자 호출시 전달한 인자
    		}
    	...
    	System.out.println("[디스패처서블릿] urlCommandMap = " + urlCommandMap);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("[디스패처서블릿] url = " + url);
		...
		switch(method) {
		case "GET" : viewName = controller.doGet(request, response);
			System.out.println("[디스패처서블릿] viewName = " + viewName); break;
