...
public class EmpUpdateController extends AbstractController {
	...
	public String doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("[이엠피업데이트컨트롤러] param = " + param);
		
		// 2. 업무로직 : 사원1명정보
		Map<String, Object> empMap = empService.selectOneEmpMap(param);
		System.out.println("[이엠피업데이트컨트롤러] empMap = " + empMap);
		
		// 직급코드목록
		List<Map<String, String>> jobList = empService.selectJobList();
		System.out.println("[이엠피업데이트컨트롤러] jobList = " + jobList);
		
		// 부서코드목록
		List<Map<String, String>> deptList = empService.selectDeptList();
		System.out.println("[이엠피업데이트컨트롤러] deptList = " + deptList);
	...
	public String doPost(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("[이엠피업데이트컨트롤러] result = " + result); // 1나오면 성공.
